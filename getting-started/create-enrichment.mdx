...
curl -X POST https://api.thred.dev/v1/enrich-chat \
...
```javascript
// Direct mode
const response = await fetch('https://api.thred.dev/v1/enrich-chat', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY',
...
// Discovery mode
const discoveryResponse = await fetch('https://api.thred.dev/v1/enrich-chat', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY',
    'Content-Type': 'application/json'
