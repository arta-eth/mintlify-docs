---
title: "Get Enriched Conversation"
...
```
GET https://api.thred.dev/v1/chat/:chatId
```
...
| --------- | ------ | -------- | ------------------------------------------- |
| `chatId`  | string | Yes      | The chat ID returned when creating the chat |
...
### When Status is "processing", "completed", or "failed"
...
```bash
curl -X GET https://api.thred.dev/v1/chat/k17f5j9m8k7j5n4m3k2j1h0g9f8e7d6c \
...
const response = await fetch(`https://api.thred.dev/v1/chat/${chatId}`, {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY'
  }
...
response = requests.get(
    f'https://api.thred.dev/v1/chat/{chat_id}',
    headers=headers
)
